...
class AdminAuthUtils extends BaseService {
  ...
  async checkLockStatus(username) {
    try {
      ...
      this.logError('检查商户账号锁定状态失败', error);
      // 出错时为了安全起见，不阻止登录
      return {
        isLocked: false,
        message: '锁定状态检查异常'
  ...
  async recordFailedAttempt(username, ip) {
    try {
      ...
      // 记录日志
      this.logAction(`商户登录失败记录: ${username}, 第${failCount}次失败, IP: ${ip}`);
      ...
      this.logError('记录商户登录失败失败', error);
      throw error;
    }
  }
  ...
  async recordSuccessfulLogin(username, ip) {
    try {
      await this.clearLockAndFailCount(username);
      // this.logAction(`商户登录成功: ${username}, IP: ${ip}`);
    } catch (error) {
      this.logError('清除商户登录失败计数失败', error);
      // 登录成功时清除失败不应该影响登录流程
    }
  }
  ...
  async clearLockAndFailCount(username) {
    try {
      ...
      this.logError('清除商户登录状态失败', error);
      throw error;
    }
  }
  ...
  async getFailedAttempts(username) {
    try {
      ...
      this.logError('获取商户登录失败次数失败', error);
      return 0;
    }
  }
}
